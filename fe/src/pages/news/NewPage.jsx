...
import {getListProducts, getListProductsByCatId} from "../../apis/products/ProductsApi";
...
const NewPage = () => {
  ...
  useEffect(() => {
    ...
    const getListProductByCatIdFnc = async () => {
  ...
  return (
    <div className="container container-list-product">
      <Helmet>
        <meta charSet="utf-8" />
        <title>{`Sản phẩm mới - SHOEDOG - Shop giày uy tín nhất TP.HCM`}</title>
        <link
          rel="canonical"
          href={CLIENT_URL+`${LIST_NEW_PRODUCT_ROUTE}?page=${new URLSearchParams(location.search).get('page')}`}
          title={`Danh sách sản phẩm mới - Shop giày uy tín nhất TP.HCM »`}
        />
        <meta
          name="description"
          content={`Danh sách sản phẩm mới. Shop giày uy tín bậc nhất TP.HCM. Chuyên hàng 2hand, hàng New chính hãng 100%. Bán giày không bán lương tâm. Chất lượng là số 1.`}
        />
      </Helmet>
      ...
      {itemInCategory.length > pageIndex ?
        <div className="text-center" style={{padding: "10px 0 0 0"}}>
          <Pagination total={itemInCategory.length} current={Number(pageCurrent)} defaultCurrent={Number(pageCurrent)} pageSize={pageIndex}
            ...
            navigate(`${LIST_NEW_PRODUCT_ROUTE}?page=${pageIndexTemp}`)
          }}/>
        </div>
        : ""
      }
